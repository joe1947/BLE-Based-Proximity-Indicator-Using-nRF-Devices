...
#define FLASH_START_ADDR 0x000F0000
...
uint32_t flash_write_addr = FLASH_START_ADDR;
uint32_t flash_read_addr = FLASH_START_ADDR;
...
int main(void)
{
    ...
    while (1) {
        ...
        {
            if (flash_write_addr >= FLASH_START_ADDR + MAX_SIZE * sizeof(adc_buf))
            {
                err = flash_erase(flash_dev, FLASH_START_ADDR, FLASH_PAGE_SIZE);
                ...
                flash_write_addr = FLASH_START_ADDR;
                flash_read_addr = FLASH_START_ADDR;
                count = 0;
            }            
            index = (flash_write_addr - FLASH_START_ADDR) / sizeof(adc_buf);
            err = adc_read(adc_channel.dev, &sequence);
            if (err == 0) {
                printk("index %d : ADC Value = %d\n",index, adc_buf);
            }
            err = flash_write(flash_dev, flash_write_addr, &adc_buf, sizeof(adc_buf));
            if (err) {
                printk("Flash write failed! Error: %d\n", err);
                return;
            }
            flash_write_addr += sizeof(adc_buf);
            count++;
...

        {
            ...
            if((count > 0) && notify_enabled)
            {
                ...
                err = flash_read(flash_dev, flash_read_addr, &value, sizeof(value));
                if (err) {
